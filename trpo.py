...
class TrpoUpdater(object):
    ...
    def assign_vars(self, theta):
        self.policy_net.sess.run(self.assign_weights_ops, feed_dict={
            self.flat_wieghts_ph: theta
        })

    def get_flat_weights(self):
        return self.policy_net.sess.run(self.flat_weights)
    ...
    def __call__(self, observes, actions, advantages):
        ...
        def get_pg():
            return self.policy_net.sess.run(self.pg, feed_dict)

        def get_hvp(p):
            feed_dict[self.p] = p
            return self.policy_net.sess.run(self.hvp, feed_dict) + self.cg_damping * p
        ...
        fullstep = stepdir / lm
        success, theta = linesearch(get_loss, prev_theta, fullstep, -pg.dot(fullstep))
        # print("success\n") if success else print("nope\n")
        self.assign_vars(theta)
        policy_loss, kl_pen = self.policy_net.sess.run([self.policy_net.surr, self.policy_net.kl_pen], feed_dict)
        ...
        #     fp = self.policy_net.sess.run(flat_pars)
        #     print(fp == theta)
